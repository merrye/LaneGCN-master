...
config["val_split"] = os.path.join(root_path, "dataset/val_0")
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.target_pred_layer = TargetPred(config)
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        # sample goals
        # candidates, candidates_mask = candidates_gather(gpu(data["tar_candts"]))
        # target_prob, offset = self.target_pred_layer(actors, actor_idcs, actor_ctrs, candidates, candidates_mask)
        # gt goal
        # target_gt = [x[0][-1, :] for x in gpu(data["gt_preds"])]
...
def candidates_gather(candidates: List[Tensor]) -> Tuple[Tensor, List[Tensor]]:
    device = candidates[0].device
    num_candidate_max = max(len(x) for x in candidates)
    output = []
    candidate_mask = []

    for x in candidates:
        l = len(x)
        output.append(torch.cat([x.float()[:, :2], torch.zeros((num_candidate_max - l, 2), device=device)]))
        candidate_mask.append(torch.cat([torch.ones(l), torch.zeros(num_candidate_max - l)]))

    output = torch.stack(output)
    candidate_mask = torch.stack(candidate_mask).to(device=device)

    # num_candidates = [len(x) for x in candidates]
    # candidates = torch.cat([x for x in candidates], 0)

    # candidate_idcs = []
    # count = 0
    # for i in range(batch_size):
    #     idcs = torch.arange(count, count + num_candidates[i]).to(candidates.device)
    #     candidate_idcs.append(idcs)
    #     count += num_candidates[i]
...
class ActorUNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.output = Res1d(n_actor, n, norm=norm, ng=ng)
...
class TargetPred(nn.Module):
    def __init__(self,
        ...
        self.prob_mlp = nn.Sequential(
            # LinearRes(in_channels + 2, hidden_dim, hidden_dim),
            LinearRes(n_actor, n_actor, norm=norm, ng=ng),
            nn.Linear(n_actor, 2 * config["num_preds"])
        )
    ...
    def forward(self, actors, actor_idcs, actor_ctrs, tar_candidate: torch.Tensor, candidate_idcs=None):    
        batch_size, n, _ = tar_candidate.size()

        # stack the target candidates to the end of input feature
        # feat_in_repeat = torch.cat([feat_in.repeat(1, n, 1), tar_candidate], dim=2) #[batch_size, N, inchannels + 2]

        # # compute probability for each candidate
        # prob_tensor = self.prob_mlp(feat_in_repeat).squeeze(2) # [batch_size, N]
        ...
        tar_candit_prob = None
        tar_offset_mean = None
