...
config["train_split"] = os.path.join(
    root_path, "dataset/train_0"
)
config["val_split"] = os.path.join(root_path, "dataset/val_00" )
config["test_split"] = os.path.join(root_path, "dataset/test_obs")

# Preprocessed Dataset
config["preprocess"] = True # whether use preprocess or not
config["preprocess_train"] = os.path.join(
    root_path, "dataset","preprocess", "train_crs_dist6_angle90_0.p"
)
config["preprocess_val"] = os.path.join(
    root_path,"dataset", "preprocess", "val_crs_dist6_angle90_0.p"
)
...
class ActorUNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.output = Res1d(n_actor, n_actor, norm="GN", ng=1)

    def forward(self, actors: Tensor) -> Tensor:
        x = actors
        enc_ftrs = self.encoder(x)

        actors = self.decoder(enc_ftrs[::-1][0], enc_ftrs[::-1][1:])
        actors = self.output(actors)[:, :, -1]
