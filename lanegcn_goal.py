...
from layers import Conv1d, Res1d, Linear, LinearRes, Null, Encoder, Decoder
...
if "save_dir" not in config:
    ...
    config["save_dir"] = os.path.join(
        root_path, "results", model_name, time_stamp
    )
...
config["pred_size"] = 30
config["pred_step"] = 1
config["num_goal_mod"] = 6
config["num_preds"] = config["pred_size"] // config["pred_step"]
config["num_mods"] = 6
config["cls_coef"] = 1.0
config["reg_coef"] = 1.0
config["goal_coef"] = 1.0
config["goal_cls_coef"] = 1.0
config["mgn"] = 0.2
config["cls_th"] = 2.0
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.actor_unet = ActorUNet(config)
        ...
        self.m2a_goal = M2A(config)
        self.a2a_goal = A2A(config)
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        actors, goal_out = self.actor_unet(actors, actor_idcs, actor_ctrs)
        
        goal_ctrs = goal_out["goal_ctrs"]
        goal_ctr = [x[:,0] for x in goal_ctrs]
        ...
        # predict goal
        # goal_out = self.goal_net(actors, actor_idcs, actor_ctrs)
        # goal_ctrs = goal_out["goal_ctrs"]
        # goal_ctr = [x[:,0] for x in goal_ctrs]
        actors = self.m2a_goal(actors, actor_idcs, goal_ctr, nodes, node_idcs, node_ctrs)   
        actors = self.a2a_goal(actors, actor_idcs, goal_ctr)

        # prediction
        out = self.pred_net(actors, actor_idcs, actor_ctrs)
        rot, orig = gpu(data["rot"]), gpu(data["orig"])
        out['goal_ctrs'] = goal_ctrs
        out['goal_cls'] = goal_out['goal_cls']
        # transform prediction to world coordinates
        for i in range(len(goal_ctrs)):
            out["goal_ctrs"][i] = torch.matmul(goal_ctrs[i], rot[i]) + orig[i].view(
                1, 1, -1
            )
...
class ActorUNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        n_in = 3
        enc_chs = [n_in, n_actor, n_actor, n_actor * 2]
        dec_chs = [n_actor * 2, n_actor, n_actor]
        self.encoder = Encoder(enc_chs)
        self.actor_decoder = Decoder(dec_chs)
        self.actor_output = Res1d(n_actor, n_actor, norm="GN", ng=1)

        self.goal_decoder = Decoder(dec_chs)
        self.goal_output = Res1d(n_actor, 2, norm="GN", ng=1)
    ...
    def forward(self, actors: Tensor, actor_idcs, actor_ctrs) -> Tensor:
        ...
        actors = self.actor_decoder(enc_ftrs[::-1][0], enc_ftrs[::-1][1:])
        actors = self.actor_output(actors)[:, :, -1]

        goals = []
        for _ in range(config['num_goal_mod']):
            goal = self.goal_decoder(enc_ftrs[::-1][0], enc_ftrs[::-1][1:])
            goals.append(self.goal_output(goal)[:, :, -1])
        ...
        goal_output = dict()
        goal_output["goal_cls"] = []
        goal_output["goal_ctrs"] = []
        for i in range(len(actor_idcs)):
            idcs = actor_idcs[i]
            goal_output["goal_cls"].append(cls[idcs])
            goal_output["goal_ctrs"].append(reg[idcs])

        return actors, goal_output
...
class PredLoss(nn.Module):
    ...
    def forward(self, out: Dict[str, List[Tensor]], gt_preds: List[Tensor], has_preds: List[Tensor]) -> Dict[str, Union[Tensor, int]]:
        cls, reg, goal_ctrs, goal_cls = out["cls"], out["reg"], out["goal_ctrs"], out["goal_cls"]
        cls = torch.cat([x for x in cls], 0)
        reg = torch.cat([x for x in reg], 0)
        goal_cls = torch.cat([x for x in goal_cls], 0)
        goal_ctrs = torch.cat([x for x in goal_ctrs], 0)

        gt_preds = torch.cat([x for x in gt_preds], 0)
        has_preds = torch.cat([x for x in has_preds], 0)

        loss_out = dict()
        zero = 0.0 * (cls.sum() + reg.sum() + goal_cls.sum() + goal_ctrs.sum())
        ...
        cls = cls[mask]
        reg = reg[mask]
        goal_cls = goal_cls[mask]
        goal_ctrs = goal_ctrs[mask]
        ...
        row_idcs = torch.arange(len(last_idcs)).long().to(last_idcs.device)
        dist = []
        for j in range(config["num_goal_mod"]):
            dist.append(
                torch.sqrt(
                    (
                        (goal_ctrs[row_idcs, j] - gt_preds[row_idcs, last_idcs])
                        ** 2
                    ).sum(1)
                )
            )
        ...
        mgn = goal_cls[row_idcs, min_idcs].unsqueeze(1) - goal_cls
        mask0 = (min_dist < self.config["cls_th"]).view(-1, 1)
        mask1 = dist - min_dist.view(-1, 1) > self.config["cls_ignore"]
        mgn = mgn[mask0 * mask1]
        mask = mgn < self.config["mgn"]
        ...
        loss_out["num_goal_cls"] += mask.sum().item()

        goal_ctrs = goal_ctrs[row_idcs, min_idcs]
        coef = self.config["goal_coef"]
        loss_out["goal_loss"] += coef * self.reg_loss(goal_ctrs[row_idcs], gt_preds[row_idcs, last_idcs])  
        loss_out["num_goal"] += len(min_idcs)
...
class Loss(nn.Module):
    ...
    def forward(self, out: Dict, data: Dict) -> Dict:
        ...
        loss_out["loss"] = loss_out["cls_loss"] / (
            ...
            loss_out["num_goal"] + 1e-10
        ) + loss_out["goal_cls_loss"] / (
            loss_out["num_goal_cls"] + 1e-10
        )
